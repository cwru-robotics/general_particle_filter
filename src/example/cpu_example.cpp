...
int main(int argc, char** argv)
{
  ...
  example_cpu_pf::objectVariance objectVariance_pf(0.15, 0.15, 0.3, 0.3);

  example_cpu_pf::PlanarParticleFilter planar_pf(particle_count, objectVariance_pf);

  example_cpu_pf::ObjectState act_state;
  example_cpu_pf::ObjectState est_state(act_state.observe(objectVariance_act));
  ...
  while (ros::ok())
  {
    ...
    example_cpu_pf::ObjectState new_state = act_state.act(obj_act, objectVariance_act);
    act_state = new_state;

    example_cpu_pf::ObjectState obs_state(act_state.observe(objectVariance_act));
