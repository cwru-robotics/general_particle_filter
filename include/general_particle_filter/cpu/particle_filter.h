...
/**
 * @brief This is a generalized particle Filter (CPU) based.
 */
...
public:
   ...
   * @param m The number of resamples taken from the weight vector.
   */
  explicit ParticleFilter(int n, int m);

  /**
   * @brief The default particle filter destructor
   */
  ~ParticleFilter();
  ...
  /**
   * @brief gets the weight of a particle at index
   *
   * @param index of the weight to get. There is no check on the index (for perfomance)
   *
   * @return the weight.
   */
